 ...
 * Targeting Singapore + SE Asia investors.
 */
...
export default async function SingaporePage() {
  ...
  const stocks = await getActiveStocks("SGX", lang, 30, true);
  const [labels, alertMap, scannedSet, recentRuns, totalSgx, priceMap] = await Promise.all([
    loadTranslations(lang),
    getAlertSummaryMap(),
    getScannedTickerSet(),
    getRecentRuns(3),
    countActiveStocks("SGX"),
    getLatestPricesForWatchlist(stocks.map((s) => ({ symbol: s.symbol, exchange: "SGX" }))),
  ]);
      ...
      >
        <div className="max-w-6xl mx-auto px-6 space-y-3">
          <h1 className="text-2xl font-bold text-white">
            {t(labels, "hero_title_sgx")}
          </h1>

          <p className="text-white/80 text-sm font-medium">
            {t(labels, "section_sgx_label")} —{" "}
            <span className="text-white font-bold">{totalSgx.toLocaleString()} {t(labels, "hero_stocks_covered")}</span>{" "}
          ...
          <div className="flex gap-3 items-center flex-wrap relative">
            ...
            <LiveScanProgress exchange="SGX" heroButton labels={labels} />
        ...
        <div>
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-3xl font-bold text-[#252525]">
              🇸🇬 {t(labels, "section_sgx_stocks")}
              <span className="text-lg font-normal text-gray-400 ml-2">{t(labels, "section_sgx_label")}</span>
            </h2>
                  ...
                  return (
                    <div key={tk.symbol} className="relative rounded-xl px-4 pt-6 pb-4 transition-all duration-200 group shadow-sm hover:-translate-y-0.5" style={cardStyle}>
                      ...
                      <div className="absolute top-1.5 right-1.5 z-10">
                        <WatchlistButton compact symbol={tk.symbol} exchange="SGX" name={tk.name} />
                      </div>
                      <Link href={`/ticker/${tk.symbol}?exchange=SGX`} className="block pr-6">
                        ...
                        <div className="mt-2 space-y-0.5">
                          {closeNum !== null && !isNaN(closeNum) ? (
                            <>
                              <div className="text-sm font-semibold text-gray-700">S${fmtSGD(closeNum)}</div>
                      ...
                      return (
                        <tr key={tk.symbol} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors" style={{ background: rowBg }}>
                          ...
                          <td className="px-4 py-3">
                            <Link href={`/ticker/${tk.symbol}?exchange=SGX`} className="font-bold hover:opacity-80" style={{ color: "#2e8b57" }}>{tk.symbol}</Link>
                          </td>
                          <td className="px-4 py-3 text-gray-500 truncate max-w-[200px]">{tk.name}</td>
                          <td className="px-4 py-3 text-right font-semibold text-gray-700">
                            {closeNum !== null && !isNaN(closeNum) ? `S$${fmtSGD(closeNum)}` : "—"}
                          </td>
